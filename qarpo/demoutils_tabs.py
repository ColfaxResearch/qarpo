...
class Interface:
        ...
        def __init__(self, config):
                ...
                self.output_info = config["job"]["output_info"]
                self.results_path = config["job"]["main_results_path"]
                self.subdir_results_path = config["job"]["subdir_results_path"] if "subdir_results_path" in config["job"] else None
        ...
        def submitJob(self, command): 
                ...
                local_res_path = self.subdir_results_path if self.subdir_results_path else None
                        ...
                        if widget["type"] == "select":
                                ...
                                if 'defines' in defines_dict:
                                        for key2, val2 in defines_dict['defines'].items():
                                                ...
                                                local_res_path = local_res_path.replace(key2, val2)
                                ...
                                for item in widget['defines']:
                                        ...
                                        local_res_path = local_res_path.replace(item, value)
                ...
                std_path = f"{self.results_path}{local_res_path}{jobid}" if self.subdir_results_path else f"{self.results_path}{jobid}"
                ...
                self.jobDict[command]['output_path'] = std_path
        ...
        def outputDisplay(self, jobid, command, min_, max_):
                ...
                progress_info = []
                progress_wid = []
                path = self.jobDict[command]['output_path']
        ...
        def outputHTML(self, result_path):
                ...
                '''
                op_list = []
                stats = f"{result_path}/{self.output_info['message_file']}"
                for file_ in os.listdir(result_path):
                        if file_.endswith(self.output_info["HTML_prefix"]):
                                op_list.append(file_)
                if os.path.isfile(stats):
                        with open(stats) as f:
                                data = json.load(f)
                        stats_line = f"<p>{data[self.output_info['message_key']]}</p>"
                else:
                        stats_line = ""
                string = ""
                height = '480' if len(op_list) == 1 else '240'

                if self.output_info["HTML_type"] == "video":
                        for x in op_list:
                            op_vid = "/user/{pwd.getpwuid(os.getuid()).pw_name}/files/{os.getcwd().split('/', 3)[3]}/{result_path}/{x}"
                            string += f"<video alt=\"\" controls autoplay height=\"{height}\"><source src=\"{op_vid}\" type=\"video/mp4\" /></video>"

                elif self.output_info["HTML_type"] == "image":
                        for x in op_list:
                            op_img = f"/user/{pwd.getpwuid(os.getuid()).pw_name}/files/{os.getcwd().split('/', 3)[3]}/{result_path}/{x}"
                            string += f"<img src='{op_img}' width='783' height='{height}'>"

                elif self.output_info["HTML_type"] == "text":
                        for x in op_list:
